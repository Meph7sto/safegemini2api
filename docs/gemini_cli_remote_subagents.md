...
1. [ADK Samples (Python)](https://github.com/google/adk-samples/tree/main/python)
2. [ADK Python Contributing Samples](https://github.com/google/adk-python/tree/main/contributing/samples)
...
If you don’t have an endpoint serving the agent card, you can provide the A2A card directly as a JSON string using `agent_card_json`.
...
Single quotes allow you to embed unescaped double quotes inside the JSON string. This format is useful for shorter, single-line JSON strings.
...
agent_card_json:
  '{ "protocolVersion": "0.3.0", "name": "Example Agent", "version": "1.0.0", "url": "dummy-url" }'
---
```

#### Using a block scalar

The literal block scalar (`|`) preserves line breaks and is highly recommended for multiline JSON strings as it avoids quote escaping entirely. The following is a complete, valid Agent Card configuration using dummy values.
...
Double quotes are also supported, but any internal double quotes in your JSON must be escaped with a backslash.
...
Many remote agents require authentication. Gemini CLI supports several authentication methods aligned with the [A2A security specification](https://a2a-protocol.org/latest/specification/#451-securityscheme). Add an `auth` block to your agent’s frontmatter to configure credentials.
...
| `http` | HTTP authentication, including Bearer token, Basic credentials, or any IANA-registered scheme. |
...
> Security tip:
> Prefer `$ENV_VAR` or `!command` over embedding secrets directly in agent files, especially for project-level agents checked into version control.
...
For any other IANA-registered scheme, for example `Digest` or `HOBA`, provide the raw authorization value.
...
Uses [Google Application Default Credentials (ADC)](https://cloud.google.com/docs/authentication/application-default-credentials) to authenticate with Google Cloud services and Cloud Run endpoints. This is the recommended auth method for agents hosted on Google Cloud infrastructure.
...
|---|---|---|
| `*.googleapis.com` | Access token | Google APIs, including Agent Engine and Vertex AI |
| `*.run.app` | Identity token | Cloud Run services |

1. Access tokens authorize API calls to Google services. They are scoped, defaulting to `cloud-platform`, and fetched via `GoogleAuth.getClient()`.
2. Identity tokens prove the caller’s identity to a service that validates the token’s audience. The audience is set to the target host. These are fetched via `GoogleAuth.getIdTokenClient()`.
...
1. Local development: Run `gcloud auth application-default login` to authenticate with your Google account.
2. CI or cloud environments: Use a service account. Set the `GOOGLE_APPLICATION_CREDENTIALS` environment variable to the path of your service account key file, or use workload identity on GKE or Cloud Run.
...
| `client_secret` | `string` | No* | OAuth client secret. Required by most authorization servers, for confidential clients. It can be omitted for public clients that don’t require a secret. |
...
1. `/agents list`: Displays all available local and remote subagents.
2. `/agents reload`: Reloads the agent registry. Use this after adding or modifying agent definition files.
3. `/agents enable <agent_name>`: Enables a specific subagent.
4. `/agents disable <agent_name>`: Disables a specific subagent.
